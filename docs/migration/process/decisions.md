...
| R25 | (R8 的结论)关键字参数**统一**用 Insight7 那套 `_wrap` 三模式 | **分层:冷路径 `argcheck`,热路径 `_wrap`,生成算子构建期静态展开** | R8 只比了「像不像 kwargs」,没比**代价**。实测 argcheck 2597 ns/call、`_wrap` 式定参包装 420 ns、裸调用 43 ns —— argcheck 和一个小 kernel(1-10 µs)同量级,**违反 D1/R1 那把「比 kernel 低 1-2 个数量级才可以忽略」的尺子**。但构造期 API 一个模型只调几百次,总计 0.5 ms,它的默认值/重载/自动 usage 白拿。**R8 没有被推翻,只是被限定了适用域** | `plan/foundations.md` §4 |
...
### 2.11 为什么 argcheck 是「采纳但限定」而不是「采纳」或「不采纳」(R25)
...
| 人的印象 | 实测 |
|---|---|
| 硬编码 Torch7 | **不成立。** 耦合共 32 行、2 处,且 `env.istype` / `env.type` 上方原注释就写着 `-- user configurable function` —— 扩展点是作者留的 |
| 过时 | **不是过时,是 Windows 上根本不能用。** `graph.lua:13` 用 `tostring(t):match('0x…')` 取地址当标识符,MSVC 的 `%p` 不带 `0x` 前缀 -> 任何带默认值的规则集在 Windows 上直接崩。**改 2 行,上游全套测试在 Windows + Lua 5.1 + 无 Torch 下全部通过** |
| 更全更好用 | **成立,而且代价也是真的。** 2597 ns/call |

所以三种极端答案都不对:

- **「不采纳」错**:它坏在一个 2 行的平台 bug 上,不是坏在设计上。为一个 2 行的 bug 放弃 842 行成熟设计不划算
- **「全面采纳」错**:2.6 µs 进不了热路径。我们自己在 D1 里用「比 kernel 低 1-2 个数量级」否掉过裸 C API 的收益,**同一把尺子必须两头都用**,否则就是双标
- **「自己再写一个」错**:那是第三套参数处理(C11 禁止),而且 usage/help 渲染那 151 行重写一遍不产生任何新价值

**边界是可判定的**:调用频次。构造期 vs 每 step。
这条边界和 `foundations.md` §2.3 的「`pl.List` vs 裸表」是**同一条边界** ——
不是新增了一个判断维度,是同一个维度上多挂了一个东西。这是采纳它的第四个理由。

**还有一条不在性能表里但同样是硬约束的**:argcheck 依赖 `debug.getupvalue` / `debug.setupvalue`。
把 `debug` 摘掉的嵌入宿主里它直接不可用。**所以 `_wrap` 那条路必须一直保留** ——
分层不只是性能选择,也是可用性兜底。
