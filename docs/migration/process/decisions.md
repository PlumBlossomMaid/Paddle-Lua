...
| R32 | ~~(我上一轮临时加的)签名层表级选项 `sizeargs`~~ **① 已被 R33 再次推翻:整个不做** | **① `sizeargs` 降级为 paddle-lua 侧 ~10 行装饰器,不进签名层;② 元素是不是整数改在转换层 `if` + `error`,类型层只查「是不是容器」** | (人的两问)「`sizeargs` 之前有讨论过吗」——**没有,是我读到 `decorator_utils.py` 之后当场加的**;「我想的是在函数里面有个 if 判断如果有小数直接 error」。① 上游自己就是**装饰器**(`decorator_utils.py:406-437`),而且全 Paddle 只用在 **5 个函数**上(`ones`/`zeros`/`empty`/`randn`/`rand`)—— 5 个函数换签名层一个永久概念不划算,而签名层的卖点之一就是小;放宿主侧,将来上游改了改动止于一个文件,反过来(库里的概念想删掉)很难。② 元素检查下放的判据是**机械**的:**去掉它之后调用仍然只有唯一解释 -> 可下放**。`IntList` 满足(`zeros{2,3}` 的 ② 里 `2` 本来就不是容器);`TensorList` 不满足(`concat{{a,b},2}` 的 ③ 正是靠「元素 `2` 不是 Tensor」出局),所以它的元素检查留在类型层。下放不是妥协而是更好:table -> `vector<int64_t>` 那步**本来就要遍历 n 次**,类型层再扫一遍是查两遍;0-D Tensor 元素(上游允许,`creation.py:1831`)也只有那一层处理得了;报错还能带下标。**但必须 `error` 不许静默取整、必须指向调用点** | `plan/argrule.md` §2.3 §2.4 |
| R33 | (R32 的结论)`sizeargs` 降级为宿主侧 ~10 行装饰器 | **整个去掉 —— 上游的兼容糖一律不移植** | (人的判断)「用得少的东西,为了 API 简洁似乎就该去掉」。决定性的不是"用得少",是**上游加这个糖的原因在 Lua 侧不存在**:Python 要写 `zeros([2,3])`,又是括号又是方括号,所以加了 `zeros(2,3)`;而 Lua 的表调用本来就省掉括号,**`zeros{2,3}` 和 `zeros(2,3)` 一样长** —— 收益是 0,成本照付(两种写法各写一遍文档、`zeros(2,"int64")` 变成反直觉的错、多一条分支要测)。同一条理由把上游整层兼容装饰器都挡在外面:**291 处参数别名**(`concat(tensors=,dim=)`,`@param_one_alias`/`@param_two_alias`)、5 处变长 size、`reshape(2,3)`/`transpose(0,1)` 等专用装饰器。别名还有两个额外的坑:`concat{x=a, tensors=b}` 要复制 291 次「不能同时给」的检查(上游 `decorator_utils.py:181-184` 抛 ValueError);`dim`/`axis` 两个名字 = index 语义标注表两条记录,漏标一条就是静默 off-by-one。**代替方案是教学式报错**(`did you mean: paddle.zeros{2, 3}`),只在错误路径上,零运行时成本 —— **教一次好过永久养一个二义入口**。例外要逐个论证,判据是「这个写法在 Lua 里比规范写法更短或更清楚吗」,**理由不能是"上游有"** | `plan/api/README.md` §2.1.3、`plan/argrule.md` §2.4 |
| R34 | (R33 的范围)逐个判断上游哪些兼容糖要移植 | **整层不判断:`decorator_utils.py`(1451 行 / 30+ 装饰器 / 68 个模块 import)全部跳过** | (人的决定)「这东西就是 Paddle 为了 PyTorch 用户用得惯才搞的,lua 里面不用考虑这些,**我们 Paddle 框架就应该用自己的语法和规范**」。证据:全文 **51 处** `PyTorch`/`torch.`,docstring 直接写 `PyTorch: torch.block_diag(x,y,z)` / `Paddle: paddle.block_diag([x,y,z])`(`:923-940`);甚至有专门劝返 torch 关键字的 `forbid_keywords`(`:517-542`)。**关键证据:35 处 `wrapper.__signature__ = inspect.signature(func)`** —— 上游自己把规范签名保留在内层,所以我们的生成器读到的**本来就是**规范签名,**忽略这层是默认行为,不是额外工作量**。⚠️ 唯一要逐个确认的:少数装饰器可能不只改参数(`legacy_reduction_decorator` / `view_decorator`),判据是「壳只动参数、内层签名不变」,发现改了行为的记 OPEN_QUESTION,不默认放行 | `plan/api/README.md` §2.1.3 |
| R35 | (R34 的范围)只跳过 `decorator_utils.py` 那层壳 | **升为硬约束 C12:用户看得见的 API 一律用 Paddle 自己的规范,不引入 PyTorch 的命名与惯例** | (人的决定)「所以我们的 paddle-lua 也不要那些 PyTorch 的东西」。**壳挡不住全部** —— 一批 torch 风格参数已经进了上游的规范签名本身:`zeros(shape, dtype, name, *, out, device, requires_grad, pin_memory)`(`creation.py:1807`),而 Paddle 自己的名字在 `to_tensor` 里原样留着(`place` / `stop_gradient`,`creation.py:1124-1129`)。同一概念两套名字,只取 Paddle 那套:`stop_gradient`(非 `requires_grad`)/ `place`(非 `device`)/ `axis`(非 `dim`)/ `x`(非 `input`/`tensors`)/ `shape`(非 `size` 参数;⚠️ `Tensor.size` 是元素总数,Paddle 原生属性,不在此列)/ `place=CUDAPinnedPlace()`(非 `pin_memory`)。**`requires_grad` 必须挡住的真正理由不是洁癖:它与 `stop_gradient` 取反、默认值也反**(`False` vs `True`),两个名字并存时任何一次搞混都是**静默地训不动** —— 没有报错,只有 loss 不降。**边界:约束的是用户看得见的表面,实现层不受限**(GC 抄 Torch7 九层 D5、schema 抄 argcheck D30 照做)。`out=` 是能力不是命名,单独记 Q-19(倾向 v1 不做)| `plan/api/README.md` §2.1.4、`CLAUDE.md` §1 C12 |
| R36 | 文档里到处写的 `paddle.f(x, { opt = 1 })`(位置参数 + 尾随选项表) | **① 没有「选项表」这个概念,只有一张参数表:`paddle.f{ x, opt = 1 }`;② 一张表内可以数组部分 + 具名键混用,同一参数给两次报错;③ 加 CI 扫文档** | (人的两句)「我觉得似乎 DataLoader 不应有额外的 `{}` 进行包装」「**你还不如直接 `{ds, batch_size=...}`**」。**这不是风格问题,那行按我们自己的规则跑不通**:`DataLoader(ds, {batch_size=64})` 是**两个实参的位置调用**,规则 #2 `batch_size:number` 收到一张 table,直接报错 —— 首屏示例是错的。`f(x, {…})` 恰恰是绝大多数 Lua 库的习惯写法,所以它会被**反复**写出来。修法不是逐处订正,是把混合表写进 `argrule` 的解析第 ① 步(数组部分按位置填前 k 条规则、具名键按名字填、重复即 error),语义与 Python「位置在前关键字在后」一致,于是**唯一正确形状就只剩 `f{…}` 一种**,没有第二种可选。⚠️ **教训:两种写法只差一个字符,人眼(包括我自己写 README、又反复读过多次)分辨不出来** —— 只能靠机器:`ci.md` 红线①b 新增 `args_no_opts_table.lua`,**且必须扫 `docs/`**,因为文档是用户抄走的第一份代码。连带订正:3 份 README、`overview.md`(4 处)、`03-codegen.md`、`11-io.md`(2 处)、`roadmap.md`、`07-serialization.md`、`research/reuse.md` | `plan/argrule.md` §2.5⑧、`plan/api/README.md` §2.1.5、`plan/ci.md` §6①b |
| R37 | 四种调用写法一视同仁,文档里随便用哪种 | **混合表 `f{x, axis=1}` 定为宿主文档的默认写法**,并由此追加两条连带约束 | (人的理由)「这种更符合 Python 那边的习惯,毕竟**大部分玩深度学习的都是从 Python 开始的**」。判据是**迁移成本**:`paddle.sum(x, axis=1, keepdim=True)` -> `paddle.sum{x, axis=1, keepdim=true}`,**只把 `(` 换成 `{`,别的一个字不动** —— 用户只需记住一条转换规则。连带(这才是真正值钱的部分):① **参数名必须与 Python 侧逐字相同**,否则「只换一个字符」的承诺当场作废 —— 这给 C12(不用 `dim`/`device`/`requires_grad`)补上了一个**用户视角**的理由,原来只有「语义会搞混」这一个;② **默认值也必须相同**,否则同一行代码两边行为不同,比报错更糟。③ 推荐混合表而不是「表内位置」`f{x,1,true}` 还有第二个理由:**Lua 数组不能有洞**,`{x, nil, true}` 的 `#t` 未定义,想跳过中间参数就没法写。⚠️ 与 Python kwargs 的不等价处必须写进用户文档:`nil` = 「没给」,表达不了显式 `None`(D30 的 3^N 后果)—— 上游是否真有这种参数尚未普查,记 **Q-21** | `plan/api/README.md` §2.1.5、`process/conventions.md` §3 §5.4 |
| R38 | 「Insight7 的 Lua 绑定 `axis` 改 1-based」(R24)是一次针对 Lua 的修补 | **升为通则:索引基准与内存序属于「绑定层」,不属于核心** —— Python 绑定 0-based / Lua 绑定 1-based / Julia 绑定 1-based + column-major,**同一个 C++ 核心** | 起因是我准备指出一个矛盾:「numpy 用户的更好选择」与 1-based 冲突 —— numpy 用户最贵的不是缺函数,是**索引重学**。去看 Insight7 目录后发现矛盾不存在:它已经是 `src/` + `bindings/{python,julia,lua}` 的架构,而 R24 当初的措辞就是"只改 **Lua 绑定**",本身已经暗含这条。**Julia 那一列是决定性证据:连内存序(column-major)都必须在绑定层处理,索引基准当然更是。** 通则化的好处是"每种语言用它自己的惯例",而不是强迫谁说别人的方言。**对本仓库的意义:我们依赖的「Insight7 Lua 绑定是 1-based」不是临时补丁,不会在 Insight7 加 Python/Julia 绑定时被回退** | `research/ecosystem.md` §3、`plan/foundations.md` §3.4 |
...
| P11 | **上游若把 `axis` 正式改名 `dim`(他们已经在加别名了),我们跟不跟?** | 跟 / 不跟 | **建议不跟。** C12 挡的是"不引入 PyTorch 那套",这条挡的是"也不要盲从上游未来的漂移"。理由是我们已经对用户承诺了两件事——**1-based 索引**、以及**与 Python 只差一个字符的迁移规则**(R37)——改名会同时打破这两个。所以「Paddle 风格」要钉死为「**2026 年这个时点的 Paddle 规范签名**(`__signature__` 里那份,不含兼容壳)」,而不是"跟着 Paddle 走"。**上游是参考,不是主人。** 这条现在不拍也行,但 P3 生成器开工前必须有答案 |
