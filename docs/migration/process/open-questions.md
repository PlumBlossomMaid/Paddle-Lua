...
| | |
|---|---|
| 现象 | `_args` 沿用 argcheck 的机制:「生成源码 -> `loadstring` -> 逐个 `debug.setupvalue` 注入 upvalue」。**这条路在 LuaJIT 上没验过。** |
| 为什么要问 | 它是 D-R26 的**命脉**。挂了不是"慢一点",是**冷路径 API 全线退回 `_wrap`** |
| 需要 | M0 #22。**纯 Lua,不需要 libpaddle,现在就能做** |
| 影响 | **必须在 P5 之前有答案。** 挂了不是翻案,是 `_args` 走解释式降级路径(§4.7 已设计这条兜底),但性能与代码形态都要重估 |

### Q-18 · `_args` 的规则表是否也用来生成 API 文档

| | |
|---|---|
| 现象 | 规则表里已经有 `help=`,argcheck 用它渲染 usage。同一份数据本可以直接产出 API 文档 |
| 倾向 | 做。**签名和文档从同一处来,是唯一能保证两者不漂移的办法** —— 否则 `plan/api/*.md` 会慢慢和代码对不上 |
| 为什么现在不定 | 要等 `_args` 的 schema 定型,而那要到 P5 |
| 影响 | 不阻塞。但**越晚做越亏**,因为手写文档会先积累起来 |

### Q-19 · `out = ` 参数要不要做

| | |
|---|---|
| 现象 | `zeros` / `ones` 等的规范签名里有 `out=`(`python/paddle/tensor/creation.py:1807`),写进调用者预分配的 Tensor。它**不只是 torch 风格的改名,是一个能力**:显存复用 |
| 与 C12 的关系 | C12 挡的是**命名**;`out` 是**语义**,所以不能靠 §2.1.4 那张表直接否掉 |
| Paddle 原生等价物 | `paddle.assign(x, output)` —— 能力已有,只是不在每个算子的签名上 |
| 倾向 | **v1 不做。** 2000+ 个生成算子每个都多一个可选参数,而收益(省一次分配)只在显存紧张的推理场景显现;真有需求时用 `paddle.assign` 或单独论证 |
| 影响 | 不阻塞。但要在 `plan/api/` 的模块文档里**明说不做**,否则会被当成遗漏反复提出 |

### Q-20 · `decorator_utils.py` 里是否有"壳改了语义"的例外

| | |
|---|---|
| 现象 | D35 决定整层跳过,依据是 35 处 `wrapper.__signature__ = inspect.signature(func)` —— 壳只动参数、内层签名不变。但个别装饰器名字看起来不像纯改参(`legacy_reduction_decorator` / `view_decorator` / `maxpool_decorator`) |
| 需要什么才能关 | 逐个读那几个 wrapper 的函数体,确认它只做参数搬运(`kwargs[a] = kwargs.pop(b)` 一类),不做 dispatch / 不改返回值 |
...
| | |
|---|---|
| 现象 | 我们的 `nil` 一律等于「没给」(D30 的 3^N 后果,`foundations.md` §4.5),所以 Python 里「传 `None`」与「不传」若含义不同,Lua 侧就**表达不出来** |
| 需要什么才能关 | 在生成器读到的规范签名上普查:默认值不是 `None`、但函数体内又对 `None` 做特判的参数。有几个?各是什么语义? |
| 倾向 | 大概率极少甚至没有(Paddle 的惯例是默认值就写 `None`,`sum` 的 `axis` 在 yaml 里默认是空 `IntArray`,`ops.yaml:5455`)。**但没查之前不许当成没有** |
| 影响 | 不阻塞 G0;P3 前查完。若真有,api 文档要逐个点名并给显式写法(如 `paddle.NONE` 哨兵),不能靠传 nil |
