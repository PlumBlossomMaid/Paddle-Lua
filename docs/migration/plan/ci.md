...
### 7.1 ⚠️ 数值对拍测不出来的那一类:**别名语义**

`reshape` / `transpose` / 切片 / `squeeze` 返回的是**视图还是拷贝** ——
**这个对拍不出来**,因为两边输出的**值完全一样**。
差别只在:改了返回值之后,**原来那个张量变没变**。

所以 `tests/parity/` 里必须有一组**专门的别名测试**,判据是三步:

```
b = f(a)          -- 可能是视图、也可能是拷贝
b[1] = 12345      -- 动一下返回值
assert a[...] == 上游 Python 侧同样三步之后的结果   -- 回头查原张量
```

这一类和 `requires_grad` / `stop_gradient` 是同一个性质:**不报错,只是结果错。**
数值对拍全绿、用户的模型静默地训出别的东西 —— 这是最贵的一种 bug,
而它恰好是"多跑几个 case"永远发现不了的。

> 同一条判据将来也适用于 Insight7 对齐 numpy(`research/ecosystem.md` §5.4)——
> numpy 自己的"视图 vs 拷贝的不确定性"是它公认的包袱之一,
> **我们扔掉它可以,但必须是"明确规定"而不是"碰巧不一样"**。
