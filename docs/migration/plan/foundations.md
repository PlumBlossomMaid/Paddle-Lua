...
### 5.4 ⚠️ `_args` 应该是独立 rock,不是 paddle-lua 的内部文件

**这条是从「生态」这个视角才看得出来的,单看 paddle-lua 看不出来。**

Insight7 的 Lua 前端**已经有一个** `_wrap` 了 —— 那是同一件事的另一个实现。
如果 paddle-lua 再内建一个 `_args`,结果是:

```lua
local ins    = require "insight"    -- 它的参数风格
local paddle = require "paddle"     -- 我们的参数风格
```

**用户在同一个脚本里要面对两套规则表方言、两种报错格式。**
这正是 C11(「基座只有一套」)要防的事 —— 只不过 C11 原本只管一个仓库内部,
**而基座的作用域本来就应该是跨仓库的**。

Insight7 的 dtype 常量和 Place 构造器已经是 Paddle 命名(§3.2),
说明「两个库在接口层对齐」这件事在这个生态里已经是既定做法。
`_args` 应该走同一条路:**独立 rock,paddle-lua 与 Insight7 都依赖它。**

⚠️ 这需要人拍板,因为它动的是 Insight7 仓库(待拍板 **P8**)。
**在拍板之前,`_args.lua` 先按「将来要能独立出去」的方式写** ——
不引用任何 paddle 内部符号,自定义类型识别通过注册接口注入(argcheck 的 `env.istype` 就是这个形状,§4.2)。
**这个约束是零成本的,而事后拆分不是。**
