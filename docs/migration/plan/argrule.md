...
softmax{t, 2}                 -- 表内位置
softmax{t, axis = 2}          -- 混合表:数组部分 + 具名键(§2.5⑧)★ 文档默认写这个
```

> ★ **混合表是宿主文档的默认写法**,因为它是 Python 那行的逐字翻译:
> `f(x, axis=2)` -> `f{x, axis=2}`,只把 `(` 换成 `{`。
> 深度学习用户几乎都是从 Python 过来的,这条约定让迁移规则只有一条
> (`api/README.md` §2.1.5)。签名层本身对四种写法一视同仁,这只是宿主的文档规范。
